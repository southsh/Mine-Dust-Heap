...
int startHoneyPots(bool enableSSH, bool enableTelnet, bool enableRDP)
{
    ...
    // 创建 SSH 线程  
    if (enableSSH) {
        rc = pthread_create(&threads[0], NULL, sshHoneyPotInit, NULL);
        if (rc) {
    ...
    // 创建 Telnet 线程（注意：可能是 telnet，而不是 telnet）  
    if (enableTelnet) {
        rc = pthread_create(&threads[1], NULL, telHoneyPotInit, NULL); // 注意：函数名可能需要改为telnetHoneyPotInit  
        if (rc) {
            fprintf(stderr, "Error: return code from pthread_create() is %d\n", rc);
            pthread_cancel(threads[0]); // 取消已创建的线程  
    ...
    // 创建 RDP 线程  
    if (enableRDP) {
        rc = pthread_create(&threads[2], NULL, rdpHoneyPotInit, NULL);
        if (rc) {
            fprintf(stderr, "Error: return code from pthread_create() is %d\n", rc);
            pthread_cancel(threads[0]); // 取消已创建的线程  
            pthread_join(threads[0], NULL); // 等待线程结束  
            if (enableTelnet) {
                pthread_cancel(threads[1]); // 取消已创建的Telnet线程  
